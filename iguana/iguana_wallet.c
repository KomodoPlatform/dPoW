...
{
    char *retstr,buf[128],wifstr[128]; cJSON *retjson; int32_t need_ZECTEST = 0,need_KMD = 0,need_BTC = 0,need_GAME = 0,need_EMC2 = 0;
    ...
    if ( waddr.wifstr[0] != 0 && bits256_nonz(waddr.privkey) != 0 && (retjson= cJSON_Parse(retstr)) != 0 )
    {
        ...
        if ( need_ZECTEST != 0 && (coin= iguana_coinfind("ZECTEST")) != 0 )
        {
            bitcoin_priv2wif(wifstr,waddr.privkey,coin->chain->wiftype);
            jaddstr(retjson,"ZECTESTwif",wifstr);
        }
